...
Errata:
	...
	The operand handling should be upgraded to allow evaluation of simple
	expressions, such as LABEL+1
...
void main(int argc, char* argv[])
{
	...
	*p = '\0';
	...
	fprintf(fpList,"Block I Apollo Guidance Computer (AGC4) assembler version 1.0\n\n");
	...
	while(fgets(buf,256,fp))
	{
		...
		if(strcmp(opcd,"")==0)
			...
			fprintf(fpList, "              %-8s %-8s %-8s %s\n",
				labl, opcd, opnd, cmnt);
		else
		{
			data = getopcode(opcd) + getopnd(opnd);
			data |= genOddParity(data) << 15;
			fprintf(fpList, "%06o %06o %-8s %-8s %-8s %s\n",
