...
regG15 MBF::register_G15; // memory buffer register bit 15
...
void MBF::execWP_RP2()
{
	register_G15.write(PAR::register_P2.read());
}

void MBF::execWP_GP()
{
	register_G15.write(PAR::gen1_15Parity(PAR::register_P.read()));
}

void MBF::execWP_WGn()
{
	register_G.write(BUS::glbl_RWBUS);
}
...
void MBF::execWP_SBEWG()
{
	...
	register_G15.write((MBF::readMemory() & 0100000) >> 15); // load bit 16 (parity) into G15
}
...
void MBF::execWP_WE()
{
	...
	MBF::writeMemory(register_G.shiftData(0, MBF::register_G.read(), MBF::conv_WE) | (register_G15.read() << 15));
}
