...
void MON::displayAGC()
{
	...
	sprintf(buf, "      S:  %04o   G:%06o   P:%06o   (r)RUN :%1d  (p)PURST:%1d (F2,F4)FCLK:%1d", 
		ADR::register_S.read(), 
		(MBF::register_G.read() & 0137777) | (MBF::register_G15.read() << 14), 
		PAR::register_P.read(),
		MON::RUN, MON::PURST, MON::FCLK);
	cout << buf << endl;

	sprintf(buf, "  RWBUS:%06o             P2:%01o        (s)STEP:%1d", 
		BUS::glbl_RWBUS & 0177777, PAR::register_P2.read(), MON::STEP);
	cout << buf << endl;

	sprintf(buf, "      B:%06o           CADR:%06o   (n)INST:%1d", 
		ALU::register_B.read(), MBF::getEffectiveAddress(), MON::INST);
	cout << buf << endl;
