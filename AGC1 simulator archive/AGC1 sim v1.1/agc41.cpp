...
void regG::execWP()		{	if(isAsserted(CLG))		write(0); 
							...
							if(isAsserted(WGn))		writeShift(glbl_RWBUS, conv_WGn);
							...
							if(isAsserted(W20))		writeShift(glbl_RWBUS, conv_W20);
							if(isAsserted(W21))		writeShift(glbl_RWBUS, conv_W21);
							if(isAsserted(W22))		writeShift(glbl_RWBUS, conv_W22);
							if(isAsserted(W23))		writeShift(glbl_RWBUS, conv_W23);
							if(isAsserted(SBEWG))	writeShift(readMemory(), conv_SBEWG);
							if(isAsserted(SBFWG))	writeShift(readMemory(), conv_SBFWG);
...
void regLP::execWP()	{	if(isAsserted(WALP))	writeShift(glbl_RWBUS, conv_WALP_LP);
							if(isAsserted(WLP) ||
							   isAsserted(WA3))		writeShift(glbl_RWBUS, conv_WLP);
						}
...
void regA::execWP()		{	if(isAsserted(WA) ||
							...
							if(isAsserted(WALP))	writeShift(glbl_RWBUS, conv_WALP_A);
						}
...
void regBNK::execRP()	{ }
void regBNK::execWP()	{ 
							if(isAsserted(WBK))		writeShift(glbl_RWBUS, conv_WBK);
						}
