 ...
 *
 *  PORTABILITY:
 *  Compiled with Microsoft Visual C++ 6.0 introductory edition. Should be fairly
...
void genAGCStates()
{
	...
	int i;
	for(i=0; CLK::registerList[i]; i++)
	{
		CLK::registerList[i]->execRP();
	}
		// Now, write the bus and any other signals into the register inputs.
	for(i=0; CLK::registerList[i]; i++)
	{
		CLK::registerList[i]->execWP();
	}
...
void toggleBreakpoint()
{
	...
	{
		char b[80];
		strcpy(b, getCommand("Set breakpoint: -- enter address (octal): "));
		cout << endl;
...
void updateAGCDisplay()
{
	...
	if(MON::CLKRATE == FCLK)
	{
		if(MON::RUN)
		{
			...
			{
				clockCounter++;
				if((TPG::register_SG.read() == TP12 && SEQ::register_SNI.read() == 1) || clockCounter > 500)
				{
					MON::displayAGC();
...
void main(int argc, char* argv[])
{
	...
	while(1)
	{
		...
		while( !_kbhit() )
		{
			...
			{
				CLK::clkAGC(); singleClock = false;
				genAGCStates();

				updateAGCDisplay();

					// This needs more work. It doesn't always stop at the
					// right time and sometimes stops at the instruction
					// afterwards, too.
