...
void MON::displayAGC()
{
	...
	sprintf(buf, "05  IN1:%06o   22    CYL:%06o   43 OUTCR2:%06o     KEY RELS:[%c]", 
		INP::register_IN1.read(), MBF::readMemory(022), MBF::readMemory(043),
		keyRels);
	cout << buf << endl;

	char checkFail = ' ';
	if(OUT::register_OUT1.read() & 0100) checkFail = '*';
	sprintf(buf, "06  IN2:%06o   23     SL:%06o   44 PIPA X:%06o   CHECK FAIL:[%c]", 
		INP::register_IN2.read(), MBF::readMemory(023), MBF::readMemory(044), checkFail);
	cout << buf << endl;
