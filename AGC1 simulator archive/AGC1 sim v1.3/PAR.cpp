...
void regP::execWP()	
{	
	if(SEQ::isAsserted(WP) || SEQ::isAsserted(WPx))	// see NOTE above
	{
			// set all bits except parity bit
		writeShift(BUS::glbl_RWBUS, PAR::conv_WP);
			// now set parity bit; in the actual AGC, this is
			// a single operation.
		if(SEQ::isAsserted(RG))
			writeField(16, 16, MBF::register_G.readField(15,15));
		else
